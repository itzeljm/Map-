...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        String regpattern = "%d. Take %s %s for %.1f miles.";
        String endpattern = "%d. Take %s %s for %.1f miles.";
        String prevName;
        String prevDirection;
        double totalLength = 0.0;
        for (int index = 0; index + 2 < segment.size(); index++) {
            int a = segment.get(index);
            int b = segment.get(index + 1);
            int c = segment.get(index + 2);
            Road first = _map.getLabel(a, b);
            String r = first.toString();
            String d = first.direction().fullName();
            double length = first.length();
            prevName = r;
            prevDirection = d;
            Road sec = _map.getLabel(b, c);
            String secName = sec.toString();
            String secdir = sec.direction().fullName();
            double lengthofsecond = sec.length();
            if (r.equals(secName) && d.equals(secdir)) {
                if (totalLength == 0.0) {
                    totalLength = length + lengthofsecond;
                } else {
                    totalLength += lengthofsecond;
                }
            } else if (totalLength != 0.0) {
                System.out.println(String.format(regpattern,
                        seq, r, d, totalLength));
                seq++; totalLength = 0.0;
            } else {
                totalLength = length;
                System.out.println(String.format(endpattern, seq,
                                prevName, prevDirection, totalLength));
                seq++; totalLength = 0.0;
            }
        }
        Road lastroad = _map.getLabel(segment.get(segment.size() - 2),
                                      segment.get(segment.size() - 1));
        String name = lastroad.toString();
        String direction = lastroad.direction().fullName();
        String end = _map.getLabel(segment.get(segment.size() - 1)).toString();
        double len  = 0.0;
        if (totalLength == 0.0) {
            len = lastroad.length();
        } else {
        ...
        System.out.println(String.format("%d. Take %s %s for %.1f miles to %s.",
                                         seq, name, direction, len, end));
        seq++; return seq;
    }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road roady = new Road(name, dir, length);
        Road reverseroady = new Road(name, dir.reverse(), length);

        _map.add(v0, v1, roady);
        _map.add(v1, v0, reverseroady);
    }
