...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("List already has commands.");
        }
    ...
    private void checkFinishedDependencies() {
        for (int index  : _depends.successors(getVertex())) {
            if (_depends.getLabel(index).isUnfinished()) {
                error("Rule is not built");
            }
    ...
    private boolean outOfDate() {
        ...
        for (int sucessor : _depends.successors(getVertex())) {
            if (_time < _depends.getLabel(sucessor).getTime()) {
                return true;
            }
        }
        return false;
